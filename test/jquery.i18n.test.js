...
(function($) {
	...
	test(
			...
			function(assert) {
				...
				var pluralAndGenderMessage = "$1 has $2 {{plural:$2|kitten|kittens}}. "
						+ "{{gender:$3|He|She}} loves to play with {{plural:$2|it|them}}.";
				assert.strictEqual(
								_(pluralAndGenderMessage, "മീര", 1, "female"),
								"മീരയ്ക്കു് ഒരു പൂച്ചക്കുട്ടി ഉണ്ടു്. അവൾ അതുമായി കളിക്കാൻ ഇഷ്ടപ്പെടുന്നു.",
								"Plural and gender test - female, singular");
				assert.strictEqual(
								_(pluralAndGenderMessage, "ഹാരി", 2, "male"),
	...
	test("Message load tests", function(assert) {
		...
		i18n.locale = 'localey';
		assert.strictEqual($.i18n().locale, "localey",
				"Locale switched to localey");
		assert.strictEqual(_('y'), "Y",
				"Message loaded for localey, message key 'y' is present");
		// Switch back to locale localex
		i18n.locale = 'localex';
		assert.strictEqual($.i18n().locale, "localex",
				"Going back-Locale is localex");
	...
	test("Message parse plural tests for Arabic", function(assert) {
		...
		assert.strictEqual(_("{{plural:$1|zero|one|two|few|many|other}}", 1),
				"one", "Arabic plural test for one");
		assert.strictEqual(_("{{plural:$1|zero|one|two|few|many|other}}", '۰'),
				"zero", "Arabic plural test for arabic digit zero");
		assert.strictEqual(_("{{plural:$1|zero|one|two|few|many|other}}", 2),
				"two", "Arabic plural test for two");
		assert.strictEqual(_("{{plural:$1|zero|one|two|few|many|other}}", 3),
				"few", "Arabic plural test for few");
		assert.strictEqual(_("{{plural:$1|zero|one|two|few|many|other}}", '۸'),
				"few", "Arabic plural test for few");
		assert.strictEqual(_("{{plural:$1|zero|one|two|few|many|other}}", 9),
				"few", "Arabic plural test for few");
		assert.strictEqual(_("{{plural:$1|zero|one|two|few|many|other}}", 110),
				"few", "Arabic plural test for few");
		assert.strictEqual(_("{{plural:$1|zero|one|two|few|many|other}}", 11),
				"many", "Arabic plural test for many");
		assert.strictEqual(_("{{plural:$1|zero|one|two|few|many|other}}", 15),
				"many", "Arabic plural test for many");
		assert.strictEqual(_("{{plural:$1|zero|one|two|few|many|other}}", 99),
				"many", "Arabic plural test for many");
		assert.strictEqual(
				_("{{plural:$1|zero|one|two|few|many|other}}", 9999), "many",
				"Arabic plural test for many");
		assert.strictEqual(_("{{plural:$1|zero|one|two|few|many|other}}", 100),
				"other", "Arabic plural test for other");
		assert.strictEqual(_("{{plural:$1|zero|one|two|few|many|other}}", 102),
				"other", "Arabic plural test for other");
		assert.strictEqual(
				_("{{plural:$1|zero|one|two|few|many|other}}", 1000), "other",
				"Arabic plural test for other");
		assert.strictEqual(_("{{plural:$1|zero|one|two|few|many|other}}", 1.7),
				"other", "Arabic plural test for other");
		assert.strictEqual(_("{{plural:$1|zero|one|two|few|many|other}}",
